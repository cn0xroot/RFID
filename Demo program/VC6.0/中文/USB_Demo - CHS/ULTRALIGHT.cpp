...
void CULTRALIGHT::OnReadCardSeriesNumber() 
{
	...
	if(re == 0)
	{
        ...
        CString str1;
		str1.Format("卡号： %02x %02x %02x %02x %02x %02x %02x\n", snr[0], snr[1], snr[2], snr[3],
			snr[4], snr[5], snr[6]);
...
void CULTRALIGHT::OnUltraLightRead() 
{
	...
	{
        CString str;
		str.Format("操作成功！卡号： %02x %02x %02x %02x %02x %02x %02x\r\n", snr[0], snr[1], snr[2], snr[3],
			snr[4], snr[5], snr[6]);

		CString str1;
		str1.Format("\r\n 接收到的数据： %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
