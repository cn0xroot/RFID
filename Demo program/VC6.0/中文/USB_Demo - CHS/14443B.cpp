...
void C14443B::OnSend()
{
	...
	{
		CString str;
		str.Format("发送成功 :%02x %02x %02x %02x %02x %02x %02x %02x", buffer[0], buffer[1], buffer[2], buffer[3], buffer[4], buffer[5], buffer[6], buffer[7]);
...
void C14443B::OnSFZUID() 
{
	...
	if(re == 0)
	{
        ...
        CString str2;
		str2.Format("卡号 : %02x %02x %02x %02x %02x %02x %02x %02x\r\n", value[0], value[1], value[2], 
...
void C14443B::OnRequestTypeB() 
{
	...
	{
		CString str;
		str.Format("操作成功！  %02x %02x %02x %02x %02x %02x %02x %02x\r\n", buffer1[0], buffer1[1], buffer1[2], buffer1[3], 
